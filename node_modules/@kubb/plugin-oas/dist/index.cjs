...
const require_generators = require('./generators-OR75jnZv.cjs');
...
function findSchemaKeyword(siblings, keyword) {
	return require_SchemaGenerator.SchemaGenerator.find(siblings, require_SchemaMapper.schemaKeywords[keyword]);
}
...
var OperationGenerator = class extends _kubb_core.BaseGenerator {
	...
	getSchemas(operation, { resolveName = (name) => name } = {}) {
		...
		const statusCodes = operation.getResponseStatusCodes().map((statusCode) => {
			...
			return {
				name: this.context.UNSTABLE_NAMING ? resolveName(_kubb_core_transformers.default.pascalCase(`${operationId} status ${name}`)) : resolveName(_kubb_core_transformers.default.pascalCase(`${operationId} ${name}`)),
		...
		return {
			...
			request: requestSchema ? {
				name: this.context.UNSTABLE_NAMING ? resolveName(_kubb_core_transformers.default.pascalCase(`${operationId} RequestData`)) : resolveName(_kubb_core_transformers.default.pascalCase(`${operationId} ${operation.method === "get" ? "queryRequest" : "mutationRequest"}`)),
			...
			response: {
				name: this.context.UNSTABLE_NAMING ? resolveName(_kubb_core_transformers.default.pascalCase(`${operationId} ResponseData`)) : resolveName(_kubb_core_transformers.default.pascalCase(`${operationId} ${operation.method === "get" ? "queryResponse" : "mutationResponse"}`)),
				operation,
				operationName,
	...
	async build(...generators) {
		...
		const generatorLimit = require_SchemaGenerator.pLimit(3);
		const operationLimit = require_SchemaGenerator.pLimit(30);
...
const pluginOas = (0, _kubb_core.definePlugin)((options) => {
	const { output = { path: "schemas" }, group, validate = true, generators = [require_generators.jsonGenerator], serverIndex, contentType, oasClass, discriminator = "strict", collisionDetection = false } = options;
...
exports.findSchemaKeyword = findSchemaKeyword;
