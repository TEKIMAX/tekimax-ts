...
import { c as TreeNode, d as _defineProperty, g as createFile, h as _classPrivateFieldInitSpec, m as _classPrivateFieldGet2, p as _classPrivateFieldSet2, t as Root } from "./Root-Dtrt_UDQ.js";
...
const loggerPlugin = definePlugin({
	...
	install(ctx, options = {}) {
		...
		ctx.on("file:processing:start", async (file, index, total) => {
			if (!state.progressBar) clack.log.step(`Processing ${pc.dim(`[${index + 1}/${total}]`)} ${formatPath(file.path)}`);
		...
		ctx.on("file:processing:end", async (file, index, total) => {
			if (state.progressBar) state.progressBar.message(`${pc.green("✓")} Finished ${pc.dim(`[${index + 1}/${total}]`)} ${formatPath(file.path)}`);
			else clack.log.success(`${pc.green("✓")} Finished ${pc.dim(`[${index + 1}/${total}]`)} ${formatPath(file.path)}`);
		...
		ctx.on("files:processing:end", async (files) => {
			...
			} else clack.log.success(`${pc.green("✓")} Processed ${pluralize("file", files.length)}`);
