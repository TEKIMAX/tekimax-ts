...
function getSchemaVersionString(schema, api) {
  if (_chunkWK3UQMKMcjs.isOpenAPI30.call(void 0, api)) {
...
function toJSONSchema(data, opts = {}) {
  ...
  const schemaAdditionalProperties = _chunkWK3UQMKMcjs.isSchema.call(void 0, schema) ? schema.additionalProperties : null;
  const {
  ...
  if (_chunkWK3UQMKMcjs.isRef.call(void 0, schema)) {
    refLogger(schema.$ref, "ref");
    return transformer({
      $ref: schema.$ref
    });
  }
  if (_chunkWK3UQMKMcjs.isSchema.call(void 0, schema, isPolymorphicAllOfChild)) {
  ...
  if (_chunkWK3UQMKMcjs.isSchema.call(void 0, schema, isPolymorphicAllOfChild)) {
    ...
    if (hasSchemaType(schema, "array")) {
      if ("items" in schema) {
        if (!Array.isArray(schema.items) && Object.keys(schema.items).length === 1 && _chunkWK3UQMKMcjs.isRef.call(void 0, schema.items)) {
  ...
  if (_chunkWK3UQMKMcjs.isSchema.call(void 0, schema, isPolymorphicAllOfChild) && globalDefaults && Object.keys(globalDefaults).length > 0 && currentLocation) {
    try {
  ...
  if (_chunkWK3UQMKMcjs.isSchema.call(void 0, schema, isPolymorphicAllOfChild) && "enum" in schema && Array.isArray(schema.enum)) {
...
function getParametersAsJSONSchema(operation, api, opts) {
  ...
  const typeKeys = _chunkKDRD2YJGcjs.getExtension.call(void 0, _chunkKDRD2YJGcjs.PARAMETER_ORDERING, api, operation).map((k) => k.toLowerCase());
...
exports.cloneObject = cloneObject; exports.isObject = isObject; exports.isPrimitive = isPrimitive; exports.matches_mimetype_default = matches_mimetype_default; exports.getParameterContentType = getParameterContentType; exports.getSchemaVersionString = getSchemaVersionString; exports.toJSONSchema = toJSONSchema; exports.types = types; exports.getParametersAsJSONSchema = getParametersAsJSONSchema;
//# sourceMappingURL=chunk-PLD2CAAP.cjs.map