...
Tekimax organizes AI capabilities into four distinct **modalities**. Each modality maps to a namespace on the client, so IDE auto-complete always shows the right methods. Ensure your provider supports the modality you are calling — unsupported calls throw a descriptive error.
