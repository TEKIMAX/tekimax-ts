...
```typescript
// The base provider is scoped strictly to text chat.
...
// Providers selectively implement capability interfaces.
...
// Example: OpenAIProvider implements almost everything.
export class OpenAIProvider implements AIProvider, VisionCapability, ImageGenerationCapability {
  // ...
}
```
